#include "../../include/clustering/initializing.h"
#include <limits>
#include <random>
...
/**
 * Implementation of the random initializing method
 * @param X The Dataset to select points from
...
/**
 * A distribution selection function as described in the slides
 * @param distances
 * @param D
 * @return
 */
std::string select_random_centroid(std::vector<std::pair<std::string, double>> distances, double D){
    ...
    int index = binary_probability_search(P, 0, P.size(), x)-1;
